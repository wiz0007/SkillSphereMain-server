...
const getAppBaseUrl = () => (process.env.FRONTEND_URL ||
    process.env.CLIENT_URL ||
    process.env.APP_URL ||
    "https://skillsphere.space").replace(/\/$/, "");
const formatDateTime = (value) => new Date(value).toLocaleString("en-IN", {
    dateStyle: "medium",
    timeStyle: "short",
});
const formatDuration = (minutes) => minutes >= 60 && minutes % 60 === 0
    ? `${minutes / 60} hour${minutes === 60 ? "" : "s"}`
    : `${minutes} minutes`;
const buildGreeting = (name) => name?.trim() || "there";
export const shouldSendSessionEmails = async (userId) => {
...
export const sendWelcomeEmail = async ({ to, name, }) => {
    await sendTransactionalEmail({
        ...
        intro: "Your account has been created successfully. Verify your email to unlock your dashboard, sessions, wallet, and tutor tools.",
        accent: "#2ec4b6",
        ctaLabel: "Open SkillSphere",
        ctaHref: `${getAppBaseUrl()}/login`,
        outro: "We’ve also sent your OTP separately, so you can confirm your account and get started right away.",
    });
};
export const sendVerificationReminderEmail = async ({ to, name, }) => {
    await sendTransactionalEmail({
        to,
        subject: "Reminder: verify your SkillSphere email",
        title: "Complete your email verification",
        intro: `${buildGreeting(name)}, your SkillSphere account is waiting for confirmation.`,
        accent: "#ffb703",
        ctaLabel: "Go to login",
        ctaHref: `${getAppBaseUrl()}/login`,
        outro: "Use the latest OTP we emailed to you. If it expired, request a fresh code from the verification screen.",
    });
};
export const sendVerificationSuccessEmail = async ({ to, name, }) => {
    await sendTransactionalEmail({
        ...
        intro: `Nice work, ${buildGreeting(name)}. Your account is now verified and ready for the full SkillSphere experience.`,
        accent: "#2ec4b6",
        ctaLabel: "Open dashboard",
        ctaHref: `${getAppBaseUrl()}/dashboard`,
        outro: "You can now explore courses, request sessions, recharge SkillCoin, and build your profile with full access.",
    });
};
export const sendVerificationReviewEmail = async ({ to, name, type, status, reviewNote, }) => {
    const titles = {
    ...
    const introMap = {
        approved: `Good news, ${buildGreeting(name)}. Your ${titles[type].toLowerCase()} has been approved.`,
    ...
    const ctaLabel = status === "approved"
        ? type === "tutor"
            ? "View profile"
            : "Open SkillSphere"
...
export const sendSessionRequestReceivedEmail = async ({ to, tutorName, studentName, courseTitle, sessionDate, durationMinutes, }) => {
    await sendTransactionalEmail({
        to,
        subject: "New session request received",
        title: "A learner requested a session",
        intro: `${buildGreeting(tutorName)}, ${studentName} has requested "${courseTitle}" on ${formatDateTime(sessionDate)}.`,
        accent: "#2ec4b6",
...
export const sendSessionStatusEmail = async ({ to, recipientName, tutorName, courseTitle, sessionDate, durationMinutes, status, }) => {
    const subjectMap = {
    ...
    await sendTransactionalEmail({
        ...
        ctaHref: `${getAppBaseUrl()}/sessions`,
        outro: `Scheduled time: ${formatDateTime(sessionDate)} · ${formatDuration(durationMinutes)}.`,
    });
};
export const sendSessionConfirmationEmail = async ({ to, tutorName, studentName, courseTitle, }) => {
    await sendTransactionalEmail({
        to,
        subject: "Session completion confirmed",
        title: "Your session has been confirmed",
        intro: `${buildGreeting(tutorName)}, ${studentName} confirmed the completion of "${courseTitle}".`,
        accent: "#2ec4b6",
...
export const sendSessionReminderEmail = async ({ to, recipientName, partnerName, courseTitle, sessionDate, durationMinutes, }) => {
    await sendTransactionalEmail({
        to,
        subject: "Reminder: your SkillSphere session starts soon",
        title: "Upcoming session reminder",
        intro: `${buildGreeting(recipientName)}, your "${courseTitle}" session with ${partnerName} is scheduled for ${formatDateTime(sessionDate)}.`,
        accent: "#ffb703",
...
export const sendRecordedCourseEmail = async ({ to, recipientName, courseTitle, actorName, status, }) => {
    const subjectMap = {
...
export const sendTuitionEmail = async ({ to, recipientName, courseTitle, actorName, status, }) => {
    const subjectMap = {
...
export const fetchUserEmailContext = async (userId) => {
