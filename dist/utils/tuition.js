...
export const buildTuitionOccurrenceDates = (schedule, fromDate, untilDate) => {
...
export const ensureTuitionSessionsGenerated = async ({ enrollment, course, dbSession, horizonDays = 60, }) => {
    ...
    const desiredDates = buildTuitionOccurrenceDates(enrollment.scheduleSnapshot, generationStart, generationHorizon);
    ...
    const existingSessions = await Session.find({
        ...
        .select("date")
        .lean();
    const existingTimestamps = new Set(existingSessions.map((session) => new Date(session.date).getTime()));
...
export const cancelFutureTuitionSessions = async ({ enrollmentId, fromDate = new Date(), dbSession, }) => {
