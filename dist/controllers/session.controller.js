    ...
    for (const session of expiredSessions) {
        ...
        try {
            ...
            await dbSession.withTransaction(async () => {
                ...
                const student = await User.findById(lockedSession.student).session(dbSession);
                if (!student) {
                    lockedSession.coinStatus = "released";
                    await lockedSession.save({ session: dbSession });
                    return;
                }
                wallet = await unlockSkillCoins(student, lockedSession.skillCoinAmount, `SkillCoin unlocked for expired request: ${lockedSession.title}`, {
    ...
    try {
        ...
        await dbSession.withTransaction(async () => {
            ...
            if (status === "cancelled" &&
                ...
                await unlockSkillCoins(student, currentSession.skillCoinAmount, `SkillCoin unlocked after session request was declined: ${currentSession.title}`, {
        ...
        return res.status([
            ...
            ? 400
            : 500).json({ message: err?.message || "Error updating session" });
    }
    finally {
        await dbSession?.endSession();
    }
