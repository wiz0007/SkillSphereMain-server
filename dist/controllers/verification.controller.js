...
const syncVerificationSummary = async (userId) => {
    const user = await User.findById(userId);
    if (!user) {
        return null;
    }
    user.verifiedBadgeLevel = deriveVerifiedBadgeLevel(user);
    await user.save();
    return user;
};
const serializeVerification = (request, profileMap) => ({
...
export const getVerificationSummary = async (req, res) => {
    try {
        ...
        const user = await User.findById(userId).select("isVerified identityVerificationStatus tutorVerificationStatus verifiedBadgeLevel");
        if (!user) {
        ...
        return res.json({
            ...
            requests: requests.map((request) => serializeVerification({
                ...request,
                user: {
                    _id: userId,
                },
...
export const submitTutorVerification = async (req, res) => {
    try {
        const userId = req.userId;
        const note = String(req.body.note || "").trim();
        const files = req.files;
        ...
        return res.status(201).json({
            ...
            summary: {
                ...
                identityVerificationStatus: synced?.identityVerificationStatus || "approved",
...
export const getAdminVerificationRequests = async (req, res) => {
    try {
...
export const reviewVerificationRequest = async (req, res) => {
    try {
        ...
        await request.save();
        const field = request.type === "identity"
        ...
        return res.json({
            ...
            summary: {
                emailVerified: Boolean(synced?.isVerified),
                identityVerificationStatus: synced?.identityVerificationStatus || "not_started",
                tutorVerificationStatus: synced?.tutorVerificationStatus || "not_started",
...
export const deverifyVerificationRequest = async (req, res) => {
    try {
        const requestId = getId(req.params.id);
        const reviewerId = req.userId;
        const reviewNote = String(req.body.reviewNote || "").trim();
