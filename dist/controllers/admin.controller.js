...
const serializeSupportMessage = (message, profileMap) => ({
...
const serializeWithdrawalRequest = (request, profileMap) => ({
...
const getActiveFinancialDependencyCount = async (userId) => {
    const ownedCourseIds = await Course.find({ tutor: userId }).distinct("_id");
    ...
    try {
        ...
        if (activeDependencies > 0) {
            return res.status(400).json({
                message: "This user has active wallet, session, course access, tuition, or withdrawal records. Resolve them before deleting the user.",
            });
        }
...
export const deleteAdminCourse = async (req, res) => {
    try {
        ...
        if (activeSessions || activeRecordedAccess || activeTuitionEnrollments) {
            return res.status(400).json({
                message: "This course has active sessions, recorded unlocks, or tuition enrollments. Resolve them before deleting the course.",
            });
        }
...
export const settleAdminSession = async (req, res) => {
    let dbSession = null;
    try {
        const id = getId(req.params.id);
        const actorId = req.userId;
        ...
        await dbSession.withTransaction(async () => {
            ...
            if (action === "release") {
                ...
                studentWallet = await unlockSkillCoins(student, session.skillCoinAmount, `SkillCoin returned after admin dispute review: ${session.title}`, {
                    sessionId: session._id,
                    ...(session.course ? { courseId: session.course } : {}),
                    extra: {
                        disputeRefund: true,
                ...
                }, dbSession || undefined, "session_unlock");
...
export const getAdminWithdrawalRequests = async (_req, res) => {
    try {
        const requests = await WithdrawalRequest.find()
            .populate("user", "username email isAdmin identityVerificationStatus tutorVerificationStatus verifiedBadgeLevel profileCompleted isVerified skillCoinBalance lockedSkillCoins createdAt")
            .populate("reviewedBy", "username email isAdmin identityVerificationStatus tutorVerificationStatus verifiedBadgeLevel profileCompleted isVerified skillCoinBalance lockedSkillCoins createdAt")
            .sort({ createdAt: -1 })
            .limit(120)
            .lean();
        const profileMap = await getProfileMap(requests.flatMap((request) => [
            request.user?._id?.toString?.() || "",
            request.reviewedBy?._id?.toString?.() || "",
        ]));
        return res.json(requests.map((request) => serializeWithdrawalRequest(request, profileMap)));
    }
    catch (error) {
...
export const updateAdminWithdrawalRequest = async (req, res) => {
    let dbSession = null;
    try {
        const id = getId(req.params.id);
        const actorId = req.userId;
        ...
        await dbSession.withTransaction(async () => {
            ...
            if (status === "processing") {
                ...
                walletSummary = await spendLockedSkillCoins(user, request.amount, `SkillCoin withdrawn to ${request.upiId}`, {
                    extra: {
                        upiId: request.upiId,
                        withdrawalRequestId: request._id.toString(),
                        adminNote,
                    },
                }, dbSession || undefined, "withdrawal_spend");
                request.status = "paid";
                ...
                walletSummary = await unlockSkillCoins(user, request.amount, `SkillCoin released after withdrawal rejection for ${request.upiId}`, {
                    extra: {
                        upiId: request.upiId,
                        withdrawalRequestId: request._id.toString(),
                        adminNote,
                    },
                }, dbSession || undefined, "withdrawal_release");
        ...
        const profileMap = await getProfileMap(populatedRequest
            ? [
...
export const getAdminSupportMessages = async (req, res) => {
    try {
        ...
        return res.json({
            ...
            messages: messages.map((message) => serializeSupportMessage(message, profileMap)),
        });
    }
    catch (error) {
...
export const getAdminWalletProof = async (req, res) => {
    try {
        const id = getId(req.params.id);
        if (!isValidObjectId(id)) {
        ...
        const proof = await getWalletTransactionProof(id, transaction.user.toString());
        if (!proof) {
