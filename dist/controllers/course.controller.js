...
const normalizeTuitionSchedule = (value) => {
    ...
    return {
        days: rawDays
            ...
            : "")
            .filter((day) => validTuitionDays.includes(day)),
        weeks: rawWeeks
            .map((week) => Number(week))
            .filter((week) => Number.isInteger(week) && week >= 1 && week <= 5),
        startTime: typeof value?.startTime === "string" ? value.startTime.trim() : "",
    };
};
...
const buildTuitionEnrollmentSummary = async (viewerId, course) => {
    ...
    const nextSession = await Session.findOne({
        ...
        .lean();
    return {
        hasEnrollment: enrollment.status === "approved" || enrollment.status === "paused",
        hasPendingRequest: enrollment.status === "pending",
        status: enrollment.status,
        requestId: enrollment._id.toString(),
        canRequest: enrollment.status === "rejected" || enrollment.status === "cancelled",
...
const buildTutorTuitionRequests = async (courseId) => {
    ...
    return requests.map((request) => ({
        ...
        createdAt: "createdAt" in request ? request.createdAt || null : null,
        approvedAt: request.approvedAt || null,
...
export const getMyTuitionEnrollments = async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId) {
            return res.status(401).json({ message: "Unauthorized" });
        ...
        const enrollments = await TuitionEnrollment.find({
            ...
            .sort({ updatedAt: -1, createdAt: -1 })
            .lean();
        const profileMap = await buildProfileMap(enrollments.flatMap((entry) => [
        ...
        return res.json(enrollments.map((entry) => {
            const courseDoc = entry.course;
            return ({
                _id: entry._id.toString(),
                role: entry.student?._id?.toString?.() === userId.toString()
                    ? "student"
                    : "tutor",
                status: entry.status,
                ...
                createdAt: "createdAt" in entry ? entry.createdAt || null : null,
    ...
    try {
        ...
        if (!viewerId) {
            return res.json({
                ...
                recordedRequests: [],
                tuitionEnrollment: course.type === "tuition" ? buildEmptyTuitionEnrollmentSummary() : null,
                tuitionRequests: [],
            });
        }
        ...
        return res.json({
            ...
            recordedRequests: isOwner && course.type === "recorded"
                ...
                : [],
            tuitionEnrollment: !isOwner && course.type === "tuition"
                ? await buildTuitionEnrollmentSummary(viewerId, course)
                : null,
            tuitionRequests: isOwner && course.type === "tuition"
...
export const deleteCourse = async (req, res) => {
    try {
        ...
        const [activeSessions, activeRecordedAccess, activeTuitionEnrollments] = await Promise.all([
        ...
        if (activeSessions || activeRecordedAccess || activeTuitionEnrollments) {
            return res.status(400).json({
                message: "This course has active bookings, unlocks, or tuition enrollments. Resolve them before deleting the course.",
            });
        }
...
export const approveRecordedCourseAccess = async (req, res) => {
    ...
    try {
        ...
        dbSession = await mongoose.startSession();
        let access = await RecordedCourseAccess.findById(accessId).session(dbSession);
        if (!access) {
        ...
        let tutor = null;
        await dbSession.withTransaction(async () => {
            const currentAccess = await RecordedCourseAccess.findById(accessId).session(dbSession);
            if (!currentAccess) {
...
export const rejectRecordedCourseAccess = async (req, res) => {
    ...
    try {
        ...
        await dbSession.withTransaction(async () => {
            ...
            await unlockSkillCoins(freshStudent, currentAccess.skillCoinAmount, `SkillCoin unlocked after recorded course rejection: ${freshCourse.title}`, {
                courseId: currentAccess.course,
                extra: {
                    accessType: "recorded_course",
                },
...
export const requestTuitionEnrollment = async (req, res) => {
    let dbSession = null;
    try {
        const userId = req.userId;
        const id = getId(req.params.id);
        ...
        if (!course.tuitionSchedule?.days?.length ||
        ...
        await dbSession.withTransaction(async () => {
            ...
            await lockSkillCoins(student, skillCoinAmount, `SkillCoin locked for tuition enrollment: ${course.title}`, {
                courseId: course._id,
                extra: {
                    enrollmentType: "tuition",
                    tutorId: course.tutor.toString(),
                },
            }, dbSession, "tuition_lock");
        ...
        const message = err?.message || "We could not start the tuition enrollment request";
        return res.status([
            "Insufficient SkillCoin balance",
...
export const approveTuitionEnrollment = async (req, res) => {
    let dbSession = null;
    try {
        const userId = req.userId;
        ...
        dbSession = await mongoose.startSession();
        let enrollment = await TuitionEnrollment.findById(enrollmentId).session(dbSession);
        if (!enrollment) {
        ...
        let tutor = null;
        await dbSession.withTransaction(async () => {
            const currentEnrollment = await TuitionEnrollment.findById(enrollmentId).session(dbSession);
...
export const rejectTuitionEnrollment = async (req, res) => {
    let dbSession = null;
    try {
        const userId = req.userId;
        const enrollmentId = getId(req.params.enrollmentId);
        ...
        await dbSession.withTransaction(async () => {
            ...
            await unlockSkillCoins(freshStudent, currentEnrollment.skillCoinAmount, `SkillCoin unlocked after tuition request was declined: ${freshCourse.title}`, {
                courseId: currentEnrollment.course,
                extra: {
                    enrollmentType: "tuition",
                },
            }, dbSession, "tuition_unlock");
...
export const pauseTuitionEnrollment = async (req, res) => {
    let dbSession = null;
    try {
        const userId = req.userId;
        const enrollmentId = getId(req.params.enrollmentId);
...
export const resumeTuitionEnrollment = async (req, res) => {
    let dbSession = null;
    try {
        const userId = req.userId;
        const enrollmentId = getId(req.params.enrollmentId);
        ...
        await dbSession.withTransaction(async () => {
            ...
            const freshCourse = await Course.findById(currentEnrollment.course).session(dbSession);
            if (!freshCourse) {
...
export const cancelTuitionEnrollment = async (req, res) => {
    let dbSession = null;
    try {
        const userId = req.userId;
        const enrollmentId = getId(req.params.enrollmentId);
        ...
        await dbSession.withTransaction(async () => {
            ...
            if (currentEnrollment.status === "pending" &&
                ...
                await unlockSkillCoins(freshStudent, currentEnrollment.skillCoinAmount, `SkillCoin unlocked after tuition request was cancelled: ${freshCourse.title}`, {
                    courseId: currentEnrollment.course,
                    extra: {
                        enrollmentType: "tuition",
                    },
