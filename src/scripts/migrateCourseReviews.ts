...
const run = async () => {
  ...
  for (const course of courses) {
    const legacyRatings = Array.isArray((course as any).ratings)
      ? (course as any).ratings
      : [];
    const legacyReviews = Array.isArray((course as any).reviews)
      ? (course as any).reviews
      : [];

    const reviewMap = new Map<
      string,
      { user: mongoose.Types.ObjectId; rating: number; comment: string }
    >();
