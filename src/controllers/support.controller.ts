...
const getSupportExecutiveEmails = () =>
  String(process.env.SUPPORT_EXECUTIVE_EMAILS || "")
    .split(",")
    .map((value) => value.trim().toLowerCase())
    .filter(Boolean);

const isSupportExecutive = async (userId: string) => {
...
const getConversationOrThrow = async (
  ...
  const allowed =
    executive || requesterId === userId || assignedId === userId;
...
export const getSupportBootstrap: RequestHandler = async (req, res) => {
  try {
    const { userId } = req;
    ...
    const unreadMap = new Map(
      unread.map((item) => [item._id.toString(), item.count])
    );
...
export const createSupportConversation: RequestHandler = async (req, res) => {
  try {
    const { userId } = req;
    ...
    const conversationPayload = serializeConversation(
...
export const getSupportMessages: RequestHandler = async (req, res) => {
  try {
    const { userId } = req;
    ...
    const profileMap = await getProfileMap(
      messages.map(
        (message) => message.sender?._id?.toString?.() || ""
      )
    );
...
export const sendSupportMessage: RequestHandler = async (req, res) => {
  try {
    const { userId } = req;
    const conversationId = getId(req.params.conversationId);
    const text = String(req.body.text || "").trim();
    ...
    const recipients = [requesterId, assignedId].filter(
      (id) => id && id !== userId
    );
...
export const updateSupportConversationStatus: RequestHandler = async (
  req,
  res
) => {
  try {
